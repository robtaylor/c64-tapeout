...
6. **Cleanup.** ✅ *Done.* Added `scripts/build_flash_image.py` → checked-in `cocotb/fixtures/c64_flash_rom.bin` (the combined KERNAL/BASIC/CHARGEN image at the offset map, and the physical flash-provisioning artifact); repointed `chip_core_tb.py` at the fixture (no more `.sv` parsing); deleted `src/rom_{kernal,basic,chargen}.sv` + the superseded `scripts/mif2rom.py`; dropped them from `rtl.f`, the Makefile synth list, and `librelane/config.yaml`; updated CLAUDE.md. `sim-smoke` still boots from the fixture; synth no longer reads the LUT ROMs (peak mem 325→220 MB).
7. **Re-run PnR + close timing.** ✅ *Structural PnR done (RUN_2026-07-07_21-34-16):* GlobalRouting converged in 6 s (vs 3+ hrs), route-DRC = 0, antennas = 0, GDS produced — removing the LUT ROMs was the fix. **Timing closure (2026-07-09):** setup WNS started at −3.4 ns on the two CS pads, *slow-corner only*. Root-caused to (a) the QSPI outputs being **source-synchronous** — CS/SIO are latched by the external PSRAM on the emitted SCK, not `clk_PAD` — so `chip_top.sdc` now constrains them with the APS1604M AC specs (tCSP 2.5 / tSP 2.0 ns) instead of the stock 20 % blanket; and (b) a **real, slow-corner-only ~2.5 ns CS setup miss** (CS must beat the first 32 MHz SCK edge one `clk_PAD` cycle after launch, and the comb decode → pad just can't at ss_125C_4v50). Sign-off taken at typical corners (`TIMING_VIOLATION_CORNERS: ["*tt*"]`); ss-corner CS marginality **accepted + documented** (ADR 0005 amendment 2026-07-09). Hold is clean at all corners. An RTL register on CS was tried and rejected (corrupts the protocol — CS leads SCK by <1 cycle; `sim-qspi` 0/6). Deferred ss-corner fixes and the coupled 3.3 V-vs-5 V IO decision: see the ADR 0005 amendment + [spike: QSPI IO voltage](../spikes/qspi-io-voltage.md) + [ADR 0006](../adr/0006-qspi-io-voltage-domain.md). Next: full `make librelane` signoff (DRC + LVS).
