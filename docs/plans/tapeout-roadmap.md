...
7. Re-run `make librelane-pdn` and trace through to PnR completion. **Confirm the `clk32` generated clock** added in WS-P2-2 task 1 (`chip_top.sdc`): this is the first PnR run since the core moved onto the divided clock, so check the STA log for the `clk32 divider pin … not found` WARNING and fix the `*clk32_div*/Q` pin path if flatten/renaming changed it — otherwise the core runs unconstrained.
